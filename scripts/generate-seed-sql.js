// Bir defalık kullanım: mevcut statik içeriği Supabase seed SQL'ine çevirir.
const fs = require("fs");
...
const site = {
  ...
  social: {
    instagram: "https://instagram.com/akturkenerji",
    linkedin: "https://linkedin.com/company/akturkenerji",
    youtube: "",
    facebook: "",
  },
};
...
const services = [
  ...
  },
  {
    ...
    summary: "Şebeke bağlantısı olmayan arazilerde güneşle çalışan sulama pompa sistemleri.",
    description:
  ...
  },
  {
    ...
    summary: "Güneş ışınımının düşük olduğu saatlerde rüzgar desteğiyle kesintisiz üretim.",
    description:
  ...
  },
  {
    ...
    summary: "Şehir dışındaki hobi bahçenizde elektrik ve su ihtiyacını güneşle karşılayın.",
    description:
  ...
  },
  {
    ...
    summary: "Panel, inverter, batarya ve bağlantı ekipmanlarında toptan/perakende tedarik.",
    description:
  ...
  },
  {
    ...
    summary: "Bölgenizde Aktürk Enerji güvencesiyle iş ortaklığı kurun.",
    description:
...
const testimonials = [
  {
    ...
    quote: "Keşiften kuruluma kadar her aşamada bilgilendirildik, süreç söz verilen tarihte tamamlandı.",
  },
  {
    name: "[PLACEHOLDER] Müşteri Adı",
    role: "Proje Müdürü, İnşaat Firması",
    quote: "Blok bazlı kurulum takvimine tam uyum sağladılar, teslim tarihimizi aksatmadılar.",
  },
  {
    name: "[PLACEHOLDER] Müşteri Adı",
    role: "İşletme Sahibi, Polatlı",
    quote: "Şebeke olmayan arazimizde sulama artık güneşle çalışıyor, yakıt maliyetimiz sıfırlandı.",
  },
];
...
sql += `insert into site_settings (id, name, short_name, founded_year, city, country, description, phone_display, phone_href, whatsapp_number, email, address_line, maps_embed_url, working_hours, instagram, linkedin, youtube, facebook, stats)
values (1, ${q(site.name)}, ${q(site.shortName)}, ${site.foundedYear}, ${q(site.city)}, ${q(site.country)}, ${q(site.description)}, ${q(site.contact.phoneDisplay)}, ${q(site.contact.phoneHref)}, ${q(site.contact.whatsappNumber)}, ${q(site.contact.email)}, ${q(site.contact.addressLine)}, ${q(site.contact.mapsEmbedUrl)}, ${q(site.contact.workingHours)}, ${q(site.social.instagram)}, ${q(site.social.linkedin)}, ${q(site.social.youtube)}, ${q(site.social.facebook)}, '${JSON.stringify(stats).replace(/'/g, "''")}'::jsonb)
...
services.forEach((s, i) => {
  sql += `insert into services (slug, eyebrow, title, summary, description, bullets, audience, sort_order)
values (${q(s.slug)}, ${q(s.eyebrow)}, ${q(s.title)}, ${q(s.summary)}, ${q(s.description)}, ${qArr(s.bullets)}, ${q(s.audience)}, ${i})
...
references.forEach((r, i) => {
  sql += `insert into project_references (slug, title, category, location, capacity, year, summary, description, sort_order)
values (${q(r.slug)}, ${q(r.title)}, ${q(r.category)}, ${q(r.location)}, ${q(r.capacity)}, ${q(r.year)}, ${q(r.summary)}, ${q(r.description)}, ${i})
on conflict (slug) do nothing;
...
testimonials.forEach((t, i) => {
  sql += `insert into testimonials (name, role, quote, sort_order)
values (${q(t.name)}, ${q(t.role)}, ${q(t.quote)}, ${i});

`;
});

blogPosts.forEach((p) => {
  sql += `insert into blog_posts (slug, title, description, category, content, published_at)
values (${q(p.slug)}, ${q(p.title)}, ${q(p.description)}, ${q(p.category)}, ${q(p.content)}, ${q(p.publishedAt)})
on conflict (slug) do nothing;

`;
});

fs.writeFileSync(path.join(__dirname, "..", "supabase", "seed.sql"), sql, "utf-8");
console.log("supabase/seed.sql written, " + sql.length + " chars");
